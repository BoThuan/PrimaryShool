...
app.controller("Ministry-ctrl", function ($scope, $http) {
  ...
  $scope.loading = function () {
    ...
    $http.get(`${API_Ministry}/MaxMaGV`).then((resp) => {
      $scope.MaxMaNS = resp.data.maGiaoVu;
      $scope.SuLyMa($scope.MaxMaNS);
    });
  };

  $scope.SuLyMa = function (maxMANS) {
    maxMANS = parseInt(maxMANS.replace("NS", ""));
    maxMANS = maxMANS + 1;
    maxMANSlength = maxMANS.toString().length;
    if (maxMANSlength < 2) {
      $scope.MaxMaNS = `NS000${maxMANS}`;
      console.log($scope.MaxMaNS);
    } else if ((maxMANSlength == 2)) {
      $scope.MaxMaNS = `NS00${maxMANS}`;
    } else if (maxMANSlength > 2) {
      $scope.MaxMaNS = `NS0${maxMANS}`;
    }
  };
  ...
  $scope.reset = function () {
    $scope.loading();
    $scope.form = {maGiaoVu: $scope.MaxMaNS, gioiTinh: true};
  };
